...
class KVSRepository:
    def __init__(self):
        self.redis = Redis(
            url=os.environ.get("UPSTASH_URL"), token=os.environ.get("UPSTASH_TOKEN")
        )
    ...
    def update_task_status(
        self, session_id: str, task_id: str, status: str, result: Optional[str] = None
    ) -> bool:
    ...
    def delete_session_data(self, session_id: str) -> bool:
        keys = [session_id, f"task_schemas:{session_id}", f"tasks:{session_id}"]
        for key in keys:
