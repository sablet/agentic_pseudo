...
class PlannerAgent:
    ...
    def _parse_user_instruction(
        ...
        # データ分析パターン
            ...
            keyword in instruction
            for keyword in ["データ分析", "予測", "モデル", "Python", "コード"]
        ):
            # データ処理タスク
        ...
        # プロジェクト管理・開発パターン
            ...
            keyword in instruction
            for keyword in ["プロジェクト", "開発", "Webサービス", "システム", "設計"]
        ):
            # 調査・企画タスク
    ...
    def _save_tasks_to_kvs(
        self, session_id: str, tasks: List[DailyTaskSchema | InfoReferenceSchema]
    ):
    ...
    def _execute_task(
        self, session_id: str, task: DailyTaskSchema | InfoReferenceSchema
    ) -> Dict[str, Any]:
        ...
        result = self.llm_agent_manager.execute_task(
            task.agent.value, task.task, context
        )
    ...
    def update_plan_dynamically(
        self, session_id: str, new_tasks: List[DailyTaskSchema | InfoReferenceSchema]
    ):
