...
from src.models.task_models import (
    TaskData, TaskSchemas, DailyTaskSchema, InfoReferenceSchema,
...
class TestMarketResearchUseCase(TestUseCaseScenarios):
    ...
    def test_market_research_hearing_setup(self, kvs_repo, planner_agent):
        ...
        # 情報収集タスクの存在確認
        info_tasks = [task for task in plan.plan if isinstance(task, InfoReferenceSchema)]
        ...
        assert "情報" in combined_descriptions or "検索" in combined_descriptions, "情報収集タスクが含まれていない"
    ...
    def test_market_research_task_dependencies(self, kvs_repo, planner_agent):
        ...
        user_instruction = "生成AI市場調査レポートを作成したい。情報収集から始めてください。"
        plan = planner_agent.create_task_plan(session_id, user_instruction)
    ...
    def test_market_research_agent_specialization(self, kvs_repo, planner_agent):
        ...
        if web_tasks:
            for task in web_tasks:
                ...
                assert ("検索" in task.task or "情報" in task.task or 
                       isinstance(task, InfoReferenceSchema)), \
                       f"Web Agentに不適切なタスクが割り当てられている: {task.task}"
        
        if casual_tasks:
            for task in casual_tasks:
                # Casual Agentはレポート作成等を担当
                assert ("レポート" in task.task or "作成" in task.task or "分析" in task.task), \
...
class TestDataAnalysisUseCase(TestUseCaseScenarios):
    ...
    def test_data_analysis_technical_workflow(self, kvs_repo, planner_agent):
        ...
        if len(plan.plan) > 0:
            ...
            has_technical_content = any(keyword in task_descriptions for keyword in technical_keywords)
    ...
    def test_coder_agent_utilization(self, kvs_repo, planner_agent):
        ...
        if len(plan.plan) > 0:
            ...
            # コード関連のタスクが含まれている
            code_related = any("コード" in task.task or "Python" in task.task or "データ" in task.task 
                             for task in plan.plan)
            
            # タスクが生成されているか、Coder Agentが使用されている
            coder_tasks = [task for task in plan.plan if task.agent == AgentType.CODER]
            
            assert code_related or len(coder_tasks) > 0, "コード関連タスクが適切に処理されていない"
...
class TestDynamicPlanningUseCase(TestUseCaseScenarios):
    ...
    def test_dynamic_plan_update_capability(self, kvs_repo, planner_agent):
        ...
        assert len(updated_task_data.daily_tasks) >= 2, "動的タスク追加が正しく保存されていない"
    ...
    def test_phased_project_execution(self, kvs_repo, planner_agent):
        ...
        phase1_instruction = "Webサービス開発プロジェクトのフェーズ1（企画・調査）を開始してください。"
        ...
        assert "調査" in phase1_tasks or "企画" in phase1_tasks or "分析" in phase1_tasks, \
...
class TestErrorHandlingAndResilience(TestUseCaseScenarios):
    ...
    def test_llm_fallback_behavior(self, kvs_repo):
        ...
        # Then: タスクが生成される
        assert planner.llm_agent_manager is not None, "LLMマネージャーが初期化されていない"
...
class TestKVSIntegration(TestUseCaseScenarios):
    ...
    def test_hearing_result_integration(self, kvs_repo, planner_agent):
        ...
        if len(plan.plan) > 0:
            ...
            has_relevant_content = any(keyword in all_tasks for keyword in fintech_keywords)
    ...
    def test_session_data_persistence(self, kvs_repo, planner_agent):
        ...
        if len(plan.plan) > 0:
            ...
            assert isinstance(saved_task_data, TaskData), "TaskDataオブジェクトが保存されていない"
