...
class TestTaskWorkflow:
    ...
    def test_complete_task_workflow_report_creation(self, planner_agent, kvs_repo):
        ...
        assert (
            len(saved_task_data.daily_tasks) > 0
            or len(saved_task_data.info_references) > 0
        )
    ...
    def test_error_handling_workflow(self, planner_agent, kvs_repo):
        ...
        task = DailyTaskSchema(
            id="error_task", agent=AgentType.WEB, task="エラーテストタスク"
        )
    ...
    def test_dynamic_plan_update(self, planner_agent, kvs_repo):
        ...
        new_tasks = [
            DailyTaskSchema(
                id="dynamic_task_1", agent=AgentType.WEB, task="動的追加タスク1"
            ),
            DailyTaskSchema(
                id="dynamic_task_2", agent=AgentType.CASUAL, task="動的追加タスク2"
            ),
        ]
