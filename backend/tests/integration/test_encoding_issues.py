...
class TestEncodingIssues:
    ...
    def test_japanese_text_not_corrupted_in_agents_api(self):
        ...
        for agent in data["agents"]:
            ...
            assert isinstance(agent, str), f"エージェント名が文字列ではありません: {type(agent)}"
            assert agent.isascii(), f"エージェント名にASCII以外の文字が含まれています: {agent}"
        
        # 説明文の日本語が正しくエンコードされていることを確認
        for agent_key, description in data["descriptions"].items():
            assert isinstance(description, str), f"説明文が文字列ではありません: {type(description)}"
    ...
    def test_task_data_encoding_integrity(self):
        ...
        for pattern in corrupted_patterns:
            matches = re.findall(pattern, response_text)
            if matches:
                pytest.fail(f"タスクデータで文字化けパターン '{pattern}' が検出されました: {matches}")
    ...
    def _check_task_encoding(self, task: dict):
        ...
        if "agent" in task:
            ...
            assert agent in expected_agents, f"予期しないエージェント名: '{agent}' (type: {type(agent)})"
            assert agent.isascii(), f"エージェント名にASCII以外の文字が含まれています: '{agent}'"
        
        # タスク名のチェック
        if "task" in task:
            task_name = task["task"]
            assert isinstance(task_name, str), f"タスク名が文字列ではありません: {type(task_name)}"
            ...
            for pattern in corrupted_patterns:
                ...
                if matches:
                    pytest.fail(f"タスク名で文字化けパターン '{pattern}' が検出されました: '{task_name}', matches: {matches}")
        
        # ステータスのチェック
        if "status" in task:
            status = task["status"]
            expected_statuses = ["pending", "in_progress", "completed", "failed", "未着手", "実行中", "完了", "失敗"]
            ...
            if status not in expected_statuses:
                ...
                for pattern in corrupted_patterns:
                    if re.search(pattern, status):
                        pytest.fail(f"ステータスで文字化けパターン '{pattern}' が検出されました: '{status}'")
                
                # 文字化けパターンが見つからない場合は警告
                print(f"警告: 予期しないステータス値: '{status}' (type: {type(status)})")
    ...
    def test_unicode_escape_sequences_not_present(self):
        ...
        if unicode_escapes:
            pytest.fail(f"レスポンスにUnicodeエスケープシーケンスが含まれています: {unicode_escapes}")
        ...
        for pattern in encoded_japanese_patterns:
            matches = re.findall(pattern, response_text)
            if matches:
                pytest.fail(f"レスポンスにエンコードされた日本語文字が含まれています: {matches}")
    ...
    def test_content_type_header_includes_utf8(self):
        ...
        assert "application/json" in content_type, f"Content-Typeにapplication/jsonが含まれていません: {content_type}"
        
        # charset=utf-8が含まれていることを確認（推奨）
        if "charset" in content_type:
            assert "utf-8" in content_type.lower(), f"Content-Typeにutf-8が含まれていません: {content_type}"
        else:
