...
from src.models.task_models import TaskData, TaskSchemas, UserSession, DailyTaskSchema, AgentType
...
class TestKVSIntegration:
    ...
    def test_planner_agent_kvs_integration(self, planner_agent_with_kvs, test_session_id):
        ...
        - 要件: タスクプランの作成と実行
        """
        planner_agent_with_kvs.kvs_repo.save_hearing_result(test_session_id, hearing_result)
        
        # When: タスクプランを作成
        user_instruction = "統合テスト用のレポートを作成してください"
        plan = planner_agent_with_kvs.create_task_plan(test_session_id, user_instruction)
