...
from src.models.task_models import TaskData, TaskSchemas, DailyTaskSchema, InfoReferenceSchema, AgentType, ReferenceType
...
class TestLLMIntegration:
    ...
    def test_task_execution_with_context(self, kvs_repo):
        ...
        # Assert
        assert "error" not in result
        assert "result" in result and ("content" in result["result"] or "output" in result["result"])
    ...
    def test_end_to_end_workflow_with_llm(self, kvs_repo):
        ...
        # Act
        # 1. タスクプラン作成
        plan = planner.create_task_plan("test_session", "市場調査レポートを作成したい。その前に必要な情報収集をしておいてほしい")
